...
export const ProductItem = ({ product, clearanceTypeOfPurchase, index }) => {
  ...
  const [typeOfPurchase, setTypeOfPurchase] = useState(() => {
    if (product.box?.countInStock > 0) {
      return "Box";
    } else if (product.each?.countInStock > 0) {
      return "Each";
    } else if (
  ...
  const { showStatusMessage, fetchUserData, setUser, user, addToGuestCart } =
    useModalContext();
  ...
  }, [product.countInStock, product.box, product.each?.countInStock]);
                ...
                : <div className='mb-2 justify-between'>
                    ...
                    <Listbox
                      ...
                      onChange={(value) => {
                        ...
                        } else if (value === "Clearance" && product.clearance) {
      ...
      {!active && (
        <div className='mb-2 flex flex-col justify-center gap-2 m-2 text-center items-center'>
          <div className='text-sm text-[#788b9b]'>
            ...
            : <>
                Sign in or register for account-specific pricing and service.{" "}
