...
export default function CheckoutWizard({ activeStep = 0 }) {
  const steps = ['User Login', 'Shipping Address', 'Payment Method', 'Confirm Order'];
    ...
    <div className="mb-8 flex items-center justify-center relative">
      ...
      {steps.map((step, index) => (
        <div key={step} className="relative z-10 flex flex-col items-center flex-1">
          ...
          <p className={`mt-2 text-sm font-semibold ${index <= activeStep ? 'text-green-700' : 'text-gray-500'}`}> 
            {step}
          </p>
        </div>
      ))}
