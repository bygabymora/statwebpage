...
describe('/api/search', () => {
    ...
    mockProducts = [
      {
        ...
        active: true,
        protected: true,
        each: { quickBooksQuantityOnHandProduction: 10, wpPrice: 5 },
        box: { quickBooksQuantityOnHandProduction: 5, wpPrice: 20 },
        slug: 'active-protected-product',
        manufacturer: 'TestManu',
        toObject: function() { return this; } // For lean()
      },
      {
        _id: '2',
        ...
        protected: true,
        each: { quickBooksQuantityOnHandProduction: 8, wpPrice: 6 },
        box: { quickBooksQuantityOnHandProduction: 3, wpPrice: 22 },
      ...
      },
      {
        ...
        active: true,
        protected: false,
        each: { quickBooksQuantityOnHandProduction: 12, wpPrice: 7 },
        box: { quickBooksQuantityOnHandProduction: 6, wpPrice: 25 },
      ...
      },
      {
        ...
        protected: false,
        each: { quickBooksQuantityOnHandProduction: 0, wpPrice: 0 }, // No stock, no price
        box: { quickBooksQuantityOnHandProduction: 0, wpPrice: 0 },
  ...
  test('should return all products for restricted users, regardless of active status', async () => {
    ...
    WpUser.findById = jest.fn().mockReturnValue({
      lean: jest.fn().mockResolvedValue({ _id: 'restrictedUserId', restricted: true }),
    });
  ...
  // Test Scenario 2: quickBooksQuantityOnHandProduction adjustment for protected products (restricted user)
  test('should adjust quickBooksQuantityOnHandProduction to 0 for protected products for restricted users', async () => {
    getToken.mockResolvedValue({ _id: 'restrictedUserId' });
    WpUser.findById = jest.fn().mockReturnValue({
      lean: jest.fn().mockResolvedValue({ _id: 'restrictedUserId', restricted: true }),
    });
    ...
    expect(protectedProduct1.each.quickBooksQuantityOnHandProduction).toBe(0);
    expect(protectedProduct1.box.quickBooksQuantityOnHandProduction).toBe(0);
    expect(protectedProduct2.each.quickBooksQuantityOnHandProduction).toBe(0);
    expect(protectedProduct2.box.quickBooksQuantityOnHandProduction).toBe(0);
  });

  // Test Scenario 3: quickBooksQuantityOnHandProduction unchanged for non-protected products (restricted user)
  test('should not change quickBooksQuantityOnHandProduction for non-protected products for restricted users', async () => {
    getToken.mockResolvedValue({ _id: 'restrictedUserId' });
    WpUser.findById = jest.fn().mockReturnValue({
      lean: jest.fn().mockResolvedValue({ _id: 'restrictedUserId', restricted: true }),
    });
    ...
    expect(nonProtectedProduct.each.quickBooksQuantityOnHandProduction).toBe(originalNonProtectedProduct.each.quickBooksQuantityOnHandProduction);
    expect(nonProtectedProduct.box.quickBooksQuantityOnHandProduction).toBe(originalNonProtectedProduct.box.quickBooksQuantityOnHandProduction);
  });

  // Test Scenario 4: quickBooksQuantityOnHandProduction unchanged (non-restricted user)
  test('should not change quickBooksQuantityOnHandProduction for any product for non-restricted users', async () => {
    getToken.mockResolvedValue({ _id: 'nonRestrictedUserId' });
    WpUser.findById = jest.fn().mockReturnValue({
      lean: jest.fn().mockResolvedValue({ _id: 'nonRestrictedUserId', restricted: false }), // Non-restricted user
    });
    ...
    responseData.forEach(rp => {
      ...
      if (rp.each) {
        expect(rp.each.quickBooksQuantityOnHandProduction).toBe(originalProduct.each.quickBooksQuantityOnHandProduction);
      }
      if (rp.box) {
        expect(rp.box.quickBooksQuantityOnHandProduction).toBe(originalProduct.box.quickBooksQuantityOnHandProduction);
      }
    });
  });
